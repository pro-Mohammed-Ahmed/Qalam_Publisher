...
(function () {
    ...
    if (typeof window.PerfectScrollbar === 'undefined') {
        console.warn('PerfectScrollbar not loaded — skipping initialization.');
        return;
    }

    // if we are on windows OS we activate the perfectScrollbar function (guarded)
    ...
    if (isWindows) {
        ...
        if (content) {
            if (typeof window.PerfectScrollbar !== 'undefined') {
                ...
                // fallback to normal scroll events if PerfectScrollbar not present
                if (navbarScrollActive == 'true') {
